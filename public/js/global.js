 ...
 * 获取多级联动的商品品牌
 */
function get_brand(id,next,select_id){
    if(id == 0) return false;
    var url = '/index.php?m=Home&c=api&a=get_brand&parent_id='+ id;
    $.ajax({
        type : "GET",
        url  : url,
        error: function(request) {
        ...
        success: function(v) {
            v = "<option value='0'>请选择商品品牌</option>" + v;
            $('#'+next).empty().html(v);
            (select_id > 0) && $('#'+next).val(select_id);//默认选中
        }
    });
 ...
 * 获取多级联动的商品系列
 */
function get_series(id,next,select_id){
    if(id == 0) return false;
    var url = '/index.php?m=Home&c=api&a=get_series&brand_id='+ id;
    $.ajax({
        type : "GET",
        url  : url,
        error: function(request) {
        ...
        success: function(v) {
            v = "<option value='0'>请选择商品系列</option>" + v;
            $('#'+next).empty().html(v);
            (select_id > 0) && $('#'+next).val(select_id);//默认选中
        }
    });
