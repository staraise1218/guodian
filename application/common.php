...
function orderBtn($order_id = 0, $order = array())
{
    ...
    if($order['pay_status'] == 0 && $order['order_status'] == 0 && $order['shipping_status'] == 0) // 待支付
    {
    ...
    if($order['pay_status'] == 0  && $order['shipping_status'] == 0 && in_array($order['order_status'], array(3, 5))){
...
function generateUsername(){
    $username = 'gd'.mt_rand(10000000, 99999999);

    if(M('users')->where('username', $username)->count()){
        $this->generateUuid();
    }
    return $username;
}
