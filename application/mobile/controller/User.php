...
class User extends Base{
    ...
    public function index(){
        ...
        $collectNum = $logic->getGoodsCollectNum($this->user_id);
...
*/    	         
    	...
    	if($res['status'] == 1){
    		...
    		setcookie('mobile',$res['result']['mobile'],null,'/');
...
//            $this->verifyHandle('user_reg');
    ...
    public function changeInfo(){
        ...
        if(false !== M('users')->where('user_id', $user_id)->setField($field, $value)){
    ...
    */
    ...
    public function bind_account()
    {
        ...
        if ($res['status'] == 1) {
            ...
            $cartLogic->setUserId($res['result']['user_id']);
            $cartLogic->doUserLoginHandle();  //用户登录后 需要对购物车 一些操作
            return $this->success("绑定成功", U('User/index'));
        }else{
    ...
    {  
        if(IS_POST){ 
            ...
            $referurl = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : U("User/index");
            if (isMobile()){
                $this->success($result['msg'], U('index/index'));
            }else{
                $this->success($result['msg'], U('index/index'));
            }
        } 
    ...
    public function bind_remove()
    {
        ...
        if ($row) {
            $this->success('解除绑定成功', U('User/bind_auth'));
        } else {
            $this->error('解除绑定失败', U('User/bind_auth'));
        }
   ...
   public  function recharge(){
    ...
    public function paypwd()
    {
        ...
        if ($step > 1) {
            $check = session('validate_code');
            if (empty($check)) {
                $this->error('验证码还未验证通过', U('User/paypwd'));
            }
        }
        if (IS_POST && $step == 3) {
            $userLogic = new UsersLogic();
            ...
            if ($data['status'] == -1)
                $this->error($data['msg']);
            //$this->success($data['msg']);
            $this->redirect(U('User/paypwd', array('step' => 3)));
            exit;
        }
        $this->assign('step', $step);
        return $this->fetch();
    ...
    public function uploadFile(){
        ...
        if($info){
            ...
            // 更新用户表
            $user_id = $this->user_id;
            M('users')->where('user_id', $user_id)->setField('head_pic', $fullPath);
            // 图片压缩
            ...
            die(json_encode(array('status'=>1, 'fullPath'=>$fullPath)));
        } else {
