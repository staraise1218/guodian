...
class Index extends Base {
    ...
    public function index(){
        ...
        $sql .= C('database.prefix')."goods_category as b on a.cat_id=b.id where a.is_recommend=1 and a.is_on_sale=1 order by a.sort limit 4";//二级分类下热卖商品
        $index_recommend_goods = S('index_recommend_goods_mobile');

        if(empty($index_recommend_goods))
        {
        	$index_recommend_goods = Db::query($sql);//首页推荐商品
        	S('index_recommend_goods_mobile',$index_recommend_goods,TPSHOP_CACHE_TIME);
        }
        $recommend_article = M('article')->where('cat_id = 8 AND is_open = 1')->order('article_id desc')->limit(0,4)->cache(true,TPSHOP_CACHE_TIME)->select();
        //获取导航
