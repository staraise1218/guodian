...
class Api extends Base {
    ...
    public function get_brand(){
        $parent_id = I('get.parent_id/d'); // 商品分类 父id
        $list = M('brand')->where("cat_id", $parent_id)->select();
        if(empty($list)){
            $list = M('brand')->where("parent_cat_id", $parent_id)->select();
        }
        foreach($list as $k => $v)
            $html .= "<option value='{$v['id']}'>{$v['name']}</option>";
        exit($html);
    ...
    public function get_series(){
        $parent_id = I('get.brand_id/d'); // 商品分类 父id
        $list = M('series')->where("brand_id", $parent_id)->select();
        foreach($list as $k => $v)
            $html .= "<option value='{$v['id']}'>{$v['name']}</option>";
        exit($html);
    }
    ...
    public function send_validate_code(){
        ...
        // }
        //判断是否已注册
        if ($scene == 1 && $type == 'mobile'){
            $users = M('users')->where('mobile',$mobile)->find();
            if($users){
                $res = array('status'=>-1,'msg'=>'该手机已注册！');
                ajaxReturn($res);
            }
        }
        if($type == 'email'){
