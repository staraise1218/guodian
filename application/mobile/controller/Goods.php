...
class Goods extends Base {
    ...
    public function goodsList(){
        ...
        $goodsLogic2 = new GoodsLogic2(); // 前台商品操作逻辑类
...
//        $replyList = M('Comment')->where(['is_show'=>1,'goods_id'=>$goods_id,'parent_id'=>['>',0]])->order("add_time desc")->select();
    ...
    public function brandList(){
        ...
        $brand = db('brand')->order('store_count !=0 desc, sort asc, goods_id desc')->group('name')->limit($page->firstRow . ',' . $page->listRows)->select();
        $goods = db('goods')->group('brand_id')->field('brand_id,goods_id')->select();
    ...
    public function brandInfo(){
        $name = I('get.name');
        $cat_id = I('get.catid');
        $brand_id = db('brand')->where(array('name'=>$name))->group('parent_cat_id')->field('id,parent_cat_id')->select();
        $cat = db('goods_category')->where('id','in',array_column($brand_id,'parent_cat_id'))->field('id,name')->select();
        if(empty($cat_id))$cat_id = $cat[0]['id'];
        $brand_ids = array_column($brand_id,'id','parent_cat_id');
        ...
        $page = new Page($count, 4, '', 3);
        $goods = db('goods')->where($condistion)->order('sort','asc')->limit($page->firstRow . ',' . $page->listRows)->select();
        $this->assign('page',$page);
    ...
    public function ajaxGoodsList(){
        $key = md5($_SERVER['REQUEST_URI'].$_SERVER['QUERY_STRING']);
        $html = S($key);
        if(!empty($html))
        {
            return json($html);
        ...
        $filter_param = array(); // 帅选数组
        $id = I('get.id/d'); // 当前分类id
        $special = I('get.special/d'); // 当前分类id
        $brand_id = I('get.brand_id',0);
        $attr = I('get.attr',''); // 属性
        $price = I('get.price',''); // 价钱
        $sort = I('get.sort'); // 排序
    ...
    public function ajax_goods_thum_images(){
        $goods_id = I('get.goods_id',0);
        $width = I('get.width',0);
        $height = I('get.height',0);
        echo file_get_contents('./'.goods_thum_images($goods_id, $width, $height));
        die;
    }
    ...
    public function ajaxBrandList(){
        ...
        $goods = db('goods')->group('brand_id')->field('brand_id,goods_id')->select();
        $data['goods'] = array_column($goods,'goods_id','brand_id');
        $data['brand'] = $brand;
        S($key,$data);
        return json($data);
    }

    public function ajaxBrandInfo(){
        $key = md5($_SERVER['REQUEST_URI'].$_SERVER['QUERY_STRING']);
        $data = S($key);
        if(!empty($data))
        {
    ...
    public function ajaxSearch()
    {
        ...
        $page = new Page($count, 4);
        if ($count > 0) {
            $data = M('goods')->where($where)->limit($page->firstRow . ',' . $page->listRows)->select();
        }
