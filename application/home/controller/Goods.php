...
class Goods extends Base {
    ...
    public function goodsInfo()
    {
        // 如果是手机跳转到 手机模版
        if(isMobile()){
            $this->redirect(U('Mobile/Goods/goodsInfo',array('id'=>I("get.id/d"))));
        }
        //C('TOKEN_ON',true);        
    ...
    public function goodsList(){
        // 如果是手机跳转到 手机模版
        if(isMobile()){
            $this->redirect(U('Mobile/Goods/goodsList'));
        }
        ...
        $special = I('get.special', 0); // 是否特卖商品
        $brand_id = I('get.brand_id',0);
        $series_id = I('get.series_id',0);//商品系列
        ...
        $series_id  && ($filter_param['series_id'] = $series_id); //加入帅选条件中
        $spec  && ($filter_param['spec'] = $spec); //加入帅选条件中
        $attr  && ($filter_param['attr'] = $attr); //加入帅选条件中
        $price  && ($filter_param['price'] = $price); //加入帅选条件中
        if($special) {
            $filter_param['special'] = 1;
            unset($filter_param['id']);
            $specialGoods = M('Goods')->where('temai', 1)->field('cat_id')->find();
            $id = $specialGoods['cat_id'];
        }; //特卖加入帅选条件中
        $goodsLogic = new GoodsLogic(); // 前台商品操作逻辑类
        $goodsLogic2 = new GoodsLogic2(); // 前台商品操作逻辑类 未加密
        ...
        if($series_id)// 系列
        {
            $goods_id_4 = $goodsLogic->getGoodsIdBySeries($series_id); // 根据 系列 查找当所有商品id
        ...
        // 特卖
        if($special){
            $goods_id_special = $goodsLogic2->getGoodsIdBySpecial(); // 根据 规格 查找当所有商品id
            $filter_goods_id = array_intersect($filter_goods_id, $goods_id_special); // 获取多个帅选条件的结果 的交集
        }
        ...
        $filter_series = $goodsLogic->get_filter_series($filter_goods_id,$filter_param,'goodsList'); // 获取指定分类下的帅选系列
        $filter_spec  = $goodsLogic2->get_filter_spec($filter_goods_id,$filter_param,'goodsList',1); // 获取指定分类下的帅选规格
        $filter_attr  = $goodsLogic->get_filter_attr($filter_goods_id,$filter_param,'goodsList',1); // 获取指定分类下的帅选属性        

        // 品牌排序
        if(is_array($filter_brand) && !empty($filter_brand)){
            $item_arr = array_column($filter_brand, 'name');
            array_multisort($item_arr, SORT_ASC, $filter_brand);
        }
        // 规格排序
        // p($filter_spec);
        if(is_array($filter_spec) && !empty($filter_spec)){
            foreach ($filter_spec as $k => &$item) {
                $item_arr = array_column($item['item'], 'item');
                array_multisort($item_arr, SORT_ASC, $item['item']);
            }
        }
        // 箱包皮具 品牌排序
        if($id == 2){
            array_multisort(array_column($filter_brand,'name'),SORT_ASC,$filter_brand);
        }
    	...
    	if(!empty($filter_attr)){
    		$filter_attr = $this->_my_sort($filter_attr,'order');
    	}
        $this->assign('filter_attr',$filter_attr);  // 帅选属性
        $this->assign('filter_brand',$filter_brand);  // 列表页帅选属性 - 商品品牌
        foreach ($filter_brand as $v){
            $filter_brand_class[$v['class']][] = $v;
        }
        is_array($filter_brand_class)?ksort($filter_brand_class):'';
        $this->assign('filter_brand_class',$filter_brand_class);  // 列表页帅选属性 - 商品品牌
        $this->assign('filter_series',$filter_series);  // 列表页帅选属性 - 商品系列
    ...
    */
    ...
    public function search()
    {
        // 如果是手机跳转到 手机模版
        if(isMobile()){
            $this->redirect(U('Mobile/Goods/search'));
        }
        //C('URL_MODEL',0);
        $filter_param = array(); // 帅选数组                        
...
//        $replyList = M('Comment')->where(['is_show'=>1,'goods_id'=>$goods_id,'parent_id'=>['>',0]])->order("add_time desc")->select();
    ...
    public function collect_goods()
    {
        ...
        if($result['status'] == '1'){
            $collect = M('goods_collect')->where(array('user_id'=>cookie('user_id')))->count();//收藏数量
            setcookie('collect',$collect,null,'/');
        }
