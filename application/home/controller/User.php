...
$this->redirect(U('Home/User/info'));
        ...
        $data = $logic->get_goods_collect($this->user_id);
    ...
    public function logout(){
        ...
        setcookie('user_id', '', time()-3600,'/');
        setcookie('is_distribut', '', time()-3600,'/');
...
*/    	         
    	...
    	if($res['status'] == 1){
    		...
    		setcookie('user_id',$res['result']['user_id'], 0,'/');
    		setcookie('mobile',$res['result']['mobile'], 0,'/');
    		setcookie('is_distribut',$res['result']['is_distribut'], 0,'/');
    		$nickname = empty($res['result']['nickname']) ? $username : $res['result']['nickname'];
            setcookie('uname',urlencode($nickname), 0,'/');
            setcookie('head_pic',$res['result']['head_pic'], 0,'/');
            setcookie('cn',0,time()-3600,'/');
            $collect = M('goods_collect')->where("user_id = ".$res['result']['user_id'])->count();//收藏数量
            setcookie('collect',$collect, 0,'/');
...
//            $this->verifyHandle('user_reg');
            ...
            $collect = M('goods_collect')->where("user_id = ".$data['result']['user_id'])->count();//收藏数量
    ...
    public function info(){
        ...
        if(IS_POST){
            ...
            I('post.province1') ? $post['province1'] = I('post.province1') : false;  //省份
            I('post.city1') ? $post['city1'] = I('post.city1') : false;  // 城市
            I('post.district1') ? $post['district1'] = I('post.district1') : false;  //地区
            I('post.address') ? $post['address'] = I('post.address') : false;  //地区
            ...
            setcookie('head_pic',$post['head_pic'],null,'/');
...
$this->assign('index_nav',NavViewLogic::getNav());//获取导航
    ...
    public function del_goods_collect(){
        $id = I('get.id');
        if(!$id)
            $this->ajaxReturn("缺少ID参数");
        $ids = explode('_',$id);
        $m =M('goods_collect');
        $m->startTrans();
        foreach ($ids as $v){
            $row = M('goods_collect')->where(array('collect_id'=>$v,'user_id'=>$this->user_id))->delete();
            if(!$row){
                $m->rollback();
        ...
        $m->commit();
        $collect = M('goods_collect')->where(array('user_id'=>$this->user_id))->count();//收藏数量
        setcookie('collect',$collect,null,'/');
        $result['status'] = 1;
    ...
    public function set_pwd(){
    	...
    	if(empty($check)){
    	    ...
    	    $res=array('status'=>0,'msg'=>'验证码还未验证通过','url'=>U('Home/User/forget_pwd'));
    		$this->ajaxReturn($res);
    	}    	
    	if(IS_POST){
    		$password = I('post.password');
    		...
    		if($password2 != $password){
                $res=array('status'=>0,'msg'=>'两次密码不一致');
                $this->ajaxReturn($res);
