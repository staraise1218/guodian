...
class LoginApi extends Base {
    ...
    {
        $data1 = $this->class_obj->respon();
        if (empty($data1)) $this->redirect(U('Home/Index/index'));
...
//            }
//        }else{
            $data = $logic->thirdLogin($data1);
//        }
        if ($data['status'] != 1){
            echo "<h3>error:</h3>" . $data['msg'];
            exit;
        }
...
setcookie('head_pic', $data1['head_pic'], null, '/');
